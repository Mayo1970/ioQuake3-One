...
#ifdef IOQ3_UWP
static qboolean IN_UWP_FieldOSK( void );
#endif
...
static void IN_GamepadMove( void )
{
	...
	uwp_in_ui   = ( catchers & ( KEYCATCH_UI | KEYCATCH_CGAME | KEYCATCH_CONSOLE | KEYCATCH_MESSAGE ) ) != 0;
	uwp_console = ( catchers & KEYCATCH_CONSOLE ) != 0;

	if ( uwp_console )
	{
		/* Right stick Y: smooth line-by-line scrolling. */
		{
			...
			if ( ry != 0 )
			{
				float fy = (float)ry / 32767.0f;
				if ( fy >  1.0f ) fy =  1.0f;
				if ( fy < -1.0f ) fy = -1.0f;
				scrollAccum += fy * 0.3f;
				int lines = (int)scrollAccum;
				scrollAccum -= (float)lines;
				while ( lines > 0 ) { Con_PageDown(); lines--; }
				while ( lines < 0 ) { Con_PageUp();   lines++; }
			}
		}

		/* D-pad Up/Down: 5-line jump (edge-triggered). */
		{
			static qboolean prevDpadUp   = qfalse;
			static qboolean prevDpadDown = qfalse;
			qboolean curUp   = SDL_GameControllerGetButton( gamepad, SDL_CONTROLLER_BUTTON_DPAD_UP )   != 0;
			qboolean curDown = SDL_GameControllerGetButton( gamepad, SDL_CONTROLLER_BUTTON_DPAD_DOWN ) != 0;
			if ( curUp && !prevDpadUp )     { int j; for (j=0;j<5;j++) Con_PageUp();   }
			if ( curDown && !prevDpadDown ) { int j; for (j=0;j<5;j++) Con_PageDown(); }
			prevDpadUp   = curUp;
			prevDpadDown = curDown;
		}

		/* LB+RB: toggle top/bottom of buffer (edge-triggered). */
		{
			static qboolean prevLB = qfalse, prevRB = qfalse;
			static int atTop = 0;
			qboolean curLB = SDL_GameControllerGetButton( gamepad, SDL_CONTROLLER_BUTTON_LEFTSHOULDER  ) != 0;
			qboolean curRB = SDL_GameControllerGetButton( gamepad, SDL_CONTROLLER_BUTTON_RIGHTSHOULDER ) != 0;
			if ( curLB && curRB && ( !prevLB || !prevRB ) )
			{
				if ( atTop ) { Con_Bottom(); atTop = 0; }
				else         { Con_Top();    atTop = 1; }
				/* Suppress LB+RB from the main button loop this frame. */
				stick_state.buttons[SDL_CONTROLLER_BUTTON_LEFTSHOULDER]  = curLB;
				stick_state.buttons[SDL_CONTROLLER_BUTTON_RIGHTSHOULDER] = curRB;
				prevLB = curLB; prevRB = curRB;
				goto uwp_console_buttons_done;
			}
			prevLB = curLB; prevRB = curRB;
		}

		/* A: show OSK (text flows as SE_CHAR into the console input line). */
		{
			static qboolean prevA = qfalse;
			qboolean curA = SDL_GameControllerGetButton( gamepad, SDL_CONTROLLER_BUTTON_A ) != 0;
			if ( curA && !prevA )
			{
				SDL_StartTextInput();
				stick_state.buttons[SDL_CONTROLLER_BUTTON_A] = curA;
				prevA = curA;
				goto uwp_console_buttons_done;
			}
			prevA = curA;
		...
		/* B: close the console. */
		{
			static qboolean prevB = qfalse;
			qboolean curB = SDL_GameControllerGetButton( gamepad, SDL_CONTROLLER_BUTTON_B ) != 0;
			if ( curB && !prevB )
			{
				Cbuf_AddText( "toggleconsole\n" );
				stick_state.buttons[SDL_CONTROLLER_BUTTON_B] = curB;
				prevB = curB;
				goto uwp_console_buttons_done;
			}
			prevB = curB;
	...
	/* LB+RB (outside console): toggle console open (edge-triggered). */
	if ( !uwp_console )
	{
		static qboolean prevLB_combo = qfalse, prevRB_combo = qfalse;
		qboolean curLB = SDL_GameControllerGetButton( gamepad, SDL_CONTROLLER_BUTTON_LEFTSHOULDER  ) != 0;
		qboolean curRB = SDL_GameControllerGetButton( gamepad, SDL_CONTROLLER_BUTTON_RIGHTSHOULDER ) != 0;
		if ( curLB && curRB && ( !prevLB_combo || !prevRB_combo ) )
		{
			Cbuf_AddText( "toggleconsole\n" );
			SDL_StartTextInput();
			stick_state.buttons[SDL_CONTROLLER_BUTTON_LEFTSHOULDER]  = curLB;
			stick_state.buttons[SDL_CONTROLLER_BUTTON_RIGHTSHOULDER] = curRB;
			prevLB_combo = curLB; prevRB_combo = curRB;
			goto uwp_console_buttons_done;
		}
		prevLB_combo = curLB;
		prevRB_combo = curRB;
	...
	/* L3+A (outside console/menu): open chat OSK (edge-triggered on A). */
	if ( !uwp_console && !uwp_in_ui )
	{
		static qboolean prevL3_chat = qfalse, prevA_chat = qfalse;
		qboolean curL3 = SDL_GameControllerGetButton( gamepad, SDL_CONTROLLER_BUTTON_LEFTSTICK ) != 0;
		qboolean curA  = SDL_GameControllerGetButton( gamepad, SDL_CONTROLLER_BUTTON_A         ) != 0;
		if ( curL3 && curA && ( !prevL3_chat || !prevA_chat ) )
		{
			Cbuf_AddText( "messagemode\n" );
			SDL_StartTextInput();
			stick_state.buttons[SDL_CONTROLLER_BUTTON_LEFTSTICK] = curL3;
			stick_state.buttons[SDL_CONTROLLER_BUTTON_A]         = curA;
			prevL3_chat = curL3; prevA_chat = curA;
			goto uwp_console_buttons_done;
		}
		prevL3_chat = curL3;
		prevA_chat  = curA;
	...
	for (i = 0; i < SDL_CONTROLLER_BUTTON_MAX; i++)
	{
		...
		if (pressed != stick_state.buttons[i])
		{
			...
			if ( uwp_in_ui && !uwp_console ) {
				switch ( sdlButton ) {
					case SDL_CONTROLLER_BUTTON_A:
						if ( pressed && IN_UWP_FieldOSK() )
							SDL_StartTextInput();
						break;
...
#ifdef IOQ3_UWP
			// Shadow LT (axis 4) and RT (axis 5) to K_JOY16/K_JOY17.
			if ( i == SDL_CONTROLLER_AXIS_TRIGGERLEFT || i == SDL_CONTROLLER_AXIS_TRIGGERRIGHT ) {
				int trigJoy = ( i == SDL_CONTROLLER_AXIS_TRIGGERLEFT ) ? K_JOY16 : K_JOY17;
				if ( !posAnalog ) {
					if ( oldAxis <= 0 && axis > 0 )
						Com_QueueEvent(in_eventTime, SE_KEY, trigJoy, qtrue, 0, NULL);
					else if ( oldAxis > 0 && axis <= 0 )
						Com_QueueEvent(in_eventTime, SE_KEY, trigJoy, qfalse, 0, NULL);
				}
			}
#endif
...
#ifdef IOQ3_UWP
	// Hide the OSK when chat/console/menu-field mode exits so it doesn't linger.
	{
		static qboolean uwp_text_active = qfalse;
		int catcher = Key_GetCatcher();
		qboolean want_text = ( catcher & ( KEYCATCH_CONSOLE | KEYCATCH_MESSAGE ) ) != 0
		                  || ( ( catcher & KEYCATCH_UI ) && Cvar_VariableIntegerValue( "ui_ime_target" ) );
...
static void IN_UWP_UpgradeDefault( const char *name, const char *stockDefault, const char *oldUwpDefault, const char *uwpDefault )
{
	const char *current = Cvar_VariableString( name );
	if ( !Q_stricmp( current, stockDefault ) || !Q_stricmp( current, oldUwpDefault ) ) {
		Cvar_Set( name, uwpDefault );
	}
}
...
static qboolean IN_UWP_FieldOSK( void )
{
	if ( !Cvar_VariableIntegerValue( "ui_ime_target" ) )
		return qfalse;
	SDL_StartTextInput();
	return qtrue;
}

static cvar_t *uwp_rumbleEnable = NULL;
static cvar_t *uwp_rumbleScale  = NULL;

void UWP_SetRumble( int low, int high, int durationMs )
{
	float scale;
	if ( !gamepad ) return;
	if ( uwp_rumbleEnable && !uwp_rumbleEnable->integer ) return;
	scale = ( uwp_rumbleScale && uwp_rumbleScale->value > 0.0f ) ? uwp_rumbleScale->value : 1.0f;
	if ( scale > 2.0f ) scale = 2.0f;
	low  = (int)( low  * scale ); if ( low  > 65535 ) low  = 65535;
	high = (int)( high * scale ); if ( high > 65535 ) high = 65535;
	SDL_GameControllerRumble( gamepad, (Uint16)low, (Uint16)high, (Uint32)durationMs );
}
#endif
...
void IN_Init( void *windowData )
{
	...
	Cvar_Get( "ui_ime_target",     "0", 0 );  // set by ui_mfield when a text field is focused
	uwp_rumbleEnable = Cvar_Get( "uwp_rumbleEnable", "1",   CVAR_ARCHIVE );
	uwp_rumbleScale  = Cvar_Get( "uwp_rumbleScale",  "1.0", CVAR_ARCHIVE );

	IN_UWP_UpgradeDefault( "j_pitch", "0.025", "0.022", "0.018" );
	IN_UWP_UpgradeDefault( "j_yaw",   "-0.025", "-0.022", "-0.018" );
	IN_UWP_OverrideDefault( "joy_threshold", "0.15",   "0.23" );

	// Clear all keyboard/mouse binds loaded from default.cfg so they don't
	// show in the Controls menu or conflict with pad binds.
	{
		int k;
		for ( k = 0; k < K_PAD0_A; k++ )
			Key_SetBinding( k, "" );
	}

	IN_UWP_SetDefaultBind( K_JOY17, "RT",        "+attack"    ); // RT   = fire
	IN_UWP_SetDefaultBind( K_JOY16, "LT",        "+zoom"      ); // LT   = zoom
	IN_UWP_SetDefaultBind( K_JOY1,  "A",         "+moveup"    ); // A    = jump
	IN_UWP_SetDefaultBind( K_JOY2,  "B",         "+movedown"  ); // B    = crouch
	IN_UWP_SetDefaultBind( K_JOY3,  "X",         "weapprev"   ); // X    = prev weapon
	IN_UWP_SetDefaultBind( K_JOY4,  "Y",         "weapnext"   ); // Y    = next weapon
	// LB (JOY10) unbound; RB = use item
	IN_UWP_SetDefaultBind( K_JOY11, "RB",        "+button2"   ); // RB   = use item
	IN_UWP_SetDefaultBind( K_JOY8,  "L3",        "+speed"     ); // L3   = walk
	IN_UWP_SetDefaultBind( K_JOY5,  "VIEW",      "+scores"    ); // VIEW = scoreboard
	IN_UWP_SetDefaultBind( K_JOY7,  "MENU_BTN",  "togglemenu" ); // MENU = pause/menu
	IN_UWP_SetDefaultBind( K_JOY12, "DPAD_UP",   "+forward"   );
	IN_UWP_SetDefaultBind( K_JOY13, "DPAD_DOWN",  "+back"     );
	IN_UWP_SetDefaultBind( K_JOY14, "DPAD_LEFT",  "+moveleft" );
	IN_UWP_SetDefaultBind( K_JOY15, "DPAD_RIGHT", "+moveright");
	// Analog stick axes still use K_PAD0_* for KeyToAxisAndSign.
...
void IN_Shutdown( void )
{
	...
	if ( gamepad ) SDL_GameControllerRumble( gamepad, 0, 0, 0 );
#endif
