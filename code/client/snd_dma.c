...
#ifdef IOQ3_UWP
extern void UWP_SetRumble( int low, int high, int durationMs );

static void UWP_RumbleForSfx( const char *name, int entityNum, qboolean localSound )
{
	if ( !name || !name[0] ) return;

	if ( localSound && strstr( name, "feedback/hit" ) ) {
		UWP_SetRumble( 10000, 25000, 90 );
		return;
	}
	if ( entityNum == listener_number && strstr( name, "/pain" ) ) {
		UWP_SetRumble( 45000, 20000, 180 );
		return;
	}
	if ( entityNum == listener_number &&
	     !Q_strncmp( name, "sound/weapons/", 14 ) &&
	     !strstr( name, "impact" ) && !strstr( name, "explode" ) ) {
		if ( strstr( name, "rocket/" ) )
			UWP_SetRumble( 55000, 45000, 160 );
		else if ( strstr( name, "shotgun/" ) )
			UWP_SetRumble( 50000, 40000, 130 );
		else
			UWP_SetRumble( 20000, 35000, 70 );
		return;
	}
}
#endif
...
#ifdef IOQ3_UWP
	UWP_RumbleForSfx( sfx->soundName, entityNum, localSound );
#endif
