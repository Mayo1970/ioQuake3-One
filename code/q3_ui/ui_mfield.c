...
#ifdef IOQ3_UWP
		/* Signal input layer that a text field is focused so A-button raises OSK. */
		trap_Cvar_SetValue( "ui_ime_target", 1 );
#endif
	}
#ifdef IOQ3_UWP
	else {
		/* Clear the signal when this field loses focus. */
		trap_Cvar_SetValue( "ui_ime_target", 0 );
	}
#endif
