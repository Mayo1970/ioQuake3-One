...
#ifdef IOQ3_UWP
// Xbox One: no keyboard defaults; controller binds are set by IN_Init at startup.
#define KB(k) -1
#else
#define KB(k) k
#endif
...
{
	{"+scores",			"show scores",		ID_SHOWSCORES,	ANIM_IDLE,		KB(K_TAB),			-1,		-1, -1},
	{"+button2",		"use item",			ID_USEITEM,		ANIM_IDLE,		KB(K_ENTER),		-1,		-1, -1},
	{"+speed",			"run / walk",		ID_SPEED,		ANIM_RUN,		KB(K_SHIFT),		-1,		-1,	-1},
	{"+forward",		"walk forward",		ID_FORWARD,		ANIM_WALK,		KB(K_UPARROW),		-1,		-1, -1},
	{"+back",			"backpedal",		ID_BACKPEDAL,	ANIM_BACK,		KB(K_DOWNARROW),	-1,		-1, -1},
	{"+moveleft",		"step left",		ID_MOVELEFT,	ANIM_STEPLEFT,	KB(','),			-1,		-1, -1},
	{"+moveright",		"step right",		ID_MOVERIGHT,	ANIM_STEPRIGHT,	KB('.'),			-1,		-1, -1},
	{"+moveup",			"up / jump",		ID_MOVEUP,		ANIM_JUMP,		KB(K_SPACE),		-1,		-1, -1},
	{"+movedown",		"down / crouch",	ID_MOVEDOWN,	ANIM_CROUCH,	KB('c'),			-1,		-1, -1},
	{"+left",			"turn left",		ID_LEFT,		ANIM_TURNLEFT,	KB(K_LEFTARROW),	-1,		-1, -1},
	{"+right",			"turn right",		ID_RIGHT,		ANIM_TURNRIGHT,	KB(K_RIGHTARROW),	-1,		-1, -1},
	{"+strafe",			"sidestep / turn",	ID_STRAFE,		ANIM_IDLE,		KB(K_ALT),			-1,		-1, -1},
	{"+lookup",			"look up",			ID_LOOKUP,		ANIM_LOOKUP,	KB(K_PGDN),			-1,		-1, -1},
	{"+lookdown",		"look down",		ID_LOOKDOWN,	ANIM_LOOKDOWN,	KB(K_DEL),			-1,		-1, -1},
	{"+mlook",			"mouse look",		ID_MOUSELOOK,	ANIM_IDLE,		KB('/'),			-1,		-1, -1},
	{"centerview",		"center view",		ID_CENTERVIEW,	ANIM_IDLE,		KB(K_END),			-1,		-1, -1},
	{"+zoom",			"zoom view",		ID_ZOOMVIEW,	ANIM_IDLE,		-1,					-1,		-1, -1},
	{"weapon 1",		"gauntlet",			ID_WEAPON1,		ANIM_WEAPON1,	KB('1'),			-1,		-1, -1},
	{"weapon 2",		"machinegun",		ID_WEAPON2,		ANIM_WEAPON2,	KB('2'),			-1,		-1, -1},
	{"weapon 3",		"shotgun",			ID_WEAPON3,		ANIM_WEAPON3,	KB('3'),			-1,		-1, -1},
	{"weapon 4",		"grenade launcher",	ID_WEAPON4,		ANIM_WEAPON4,	KB('4'),			-1,		-1, -1},
	{"weapon 5",		"rocket launcher",	ID_WEAPON5,		ANIM_WEAPON5,	KB('5'),			-1,		-1, -1},
	{"weapon 6",		"lightning",		ID_WEAPON6,		ANIM_WEAPON6,	KB('6'),			-1,		-1, -1},
	{"weapon 7",		"railgun",			ID_WEAPON7,		ANIM_WEAPON7,	KB('7'),			-1,		-1, -1},
	{"weapon 8",		"plasma gun",		ID_WEAPON8,		ANIM_WEAPON8,	KB('8'),			-1,		-1, -1},
	{"weapon 9",		"BFG",				ID_WEAPON9,		ANIM_WEAPON9,	KB('9'),			-1,		-1, -1},
	{"+attack",			"attack",			ID_ATTACK,		ANIM_ATTACK,	KB(K_CTRL),			-1,		-1, -1},
	{"weapprev",		"prev weapon",		ID_WEAPPREV,	ANIM_IDLE,		KB('['),			-1,		-1, -1},
	{"weapnext",		"next weapon",		ID_WEAPNEXT,	ANIM_IDLE,		KB(']'),			-1,		-1, -1},
	{"+button3",		"gesture",			ID_GESTURE,		ANIM_GESTURE,	KB(K_MOUSE3),		-1,		-1, -1},
	{"messagemode",		"chat",				ID_CHAT,		ANIM_CHAT,		KB('t'),			-1,		-1, -1},
	{"messagemode2",	"chat - team",		ID_CHAT2,		ANIM_CHAT,		-1,					-1,		-1, -1},
	{"messagemode3",	"chat - target",	ID_CHAT3,		ANIM_CHAT,		-1,					-1,		-1, -1},
	{"messagemode4",	"chat - attacker",	ID_CHAT4,		ANIM_CHAT,		-1,					-1,		-1, -1},
	{"togglemenu",		"toggle menu",		ID_TOGGLEMENU,	ANIM_IDLE,		KB(K_ESCAPE),		-1,		-1, -1},
...
#ifdef IOQ3_UWP
	// Only show gamepad bindings; keyboard bindings are irrelevant on Xbox.
	// Prefer whichever of b1/b2 is a K_PAD0_* key.
	{
		int padKey = -1;
		if (b1 >= K_PAD0_A && b1 <= K_PAD0_TOUCHPAD)
			padKey = b1;
		else if (b2 >= K_PAD0_A && b2 <= K_PAD0_TOUCHPAD)
			padKey = b2;
		...
		{
			trap_Key_KeynumToStringBuf(padKey, name, 32);
			Q_strupr(name);
		}
	}
#else
...
#ifdef IOQ3_UWP
	// Scan the full key range so K_PAD0_* keys (>= 256) are found.
	// Put pad keys into bind1 first so the display logic finds them.
	for ( j = K_PAD0_A; j < MAX_KEYS; j++ )
	{
...
#ifdef IOQ3_UWP
		// Drop any keyboard key from the local store; only pad keys are
		// relevant on Xbox. This also prevents keyboard binds from
		// overwriting pad binds when Controls_SetConfig writes them back.
		if (bindptr->bind1 != -1 && (bindptr->bind1 < K_PAD0_A || bindptr->bind1 > K_PAD0_TOUCHPAD))
			bindptr->bind1 = bindptr->bind2 >= K_PAD0_A && bindptr->bind2 <= K_PAD0_TOUCHPAD ? bindptr->bind2 : -1;
		if (bindptr->bind2 != -1 && (bindptr->bind2 < K_PAD0_A || bindptr->bind2 > K_PAD0_TOUCHPAD))
			bindptr->bind2 = -1;
#endif
	}
...
#ifdef IOQ3_UWP
			case K_ENTER:
				// K_ENTER is mirrored from A; if we're not waiting, swallow it
				// here so it doesn't re-trigger QM_ACTIVATED after a pad bind.
				goto ignorekey;
#endif
...
#ifdef IOQ3_UWP
				trap_Cvar_Set( "ui_waitingforkey", "0" );
#endif
				Controls_Update();
...
#ifdef IOQ3_UWP
			case K_ENTER:
				// A button sends K_ENTER as a UI mirror; treat it as K_PAD0_A
				// so the A button can be bound and doesn't fall through to ignorekey.
				key = K_PAD0_A;
				break;
#endif
...
#ifdef IOQ3_UWP
		trap_Cvar_Set( "ui_waitingforkey", "1" );
#endif
		Controls_Update();
	}
}
